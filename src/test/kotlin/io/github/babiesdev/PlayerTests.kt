...
class PlayerTests {
    ...
    fun `한번에 최대 10칸 이내로 왼쪽으로 움직일 수 있다`(moveCount: Int) {
        val rabbit: Participant = Player(Position(10))

        val movedRabbit = rabbit.leftMove(moveCount)

        assertThat(movedRabbit.position).isEqualTo(Position(10 - moveCount))
    }
    ...
    @ParameterizedTest
    @ValueSource(ints = [1, 5])
    fun `한번에 10칸을 초과해서 오른쪽으로 이동할 경우 예외가 발생한다`(moveCount: Int) {
        val rabbit: Participant = Player(Position(0))
    ...
    @ParameterizedTest
    @ValueSource(ints = [1, 5])
    fun `한번에 10칸을 초과해서 왼쪽으로 이동할 경우 예외가 발생한다`(moveCount: Int) {
        val rabbit: Participant = Player(Position(0))
